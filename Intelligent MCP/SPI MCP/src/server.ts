      ...
      case "tools/list":
        return ok(request.id, {
          tools: [
            ...
            {
              name: "run_most_used_sales_play_programs",
              description:
                "Preferred path for 'most used sales play tagged in number of programs'. Runs CRMA-equivalent weighted ACV/PG logic.",
              inputSchema: {
                type: "object",
                properties: {
                  fiscal_year: { type: "string" },
...
async function handleToolCall(params: Record<string, unknown>): Promise<unknown> {
  ...
  if (toolName === "run_most_used_sales_play_programs") {
    const originalQuery = String(args.original_query ?? "");
    if (!isScopeConfirmedByQuery(originalQuery, String(args.fiscal_year ?? ""), String(args.fiscal_quarter ?? ""))) {
      return {
        content: [
            ...
            text: JSON.stringify(
              {
                clarification_needed: true,
                message:
                  "Please confirm fiscal scope in your question (example: 'most used sales play tagged in number of programs FY27 Q2')."
              },
              null,
              2
            )
    ...
    return handleToolCall({
      name: "run_formula_metric",
      arguments: {
        metric_id: "spm_most_used_sales_play_programs",
        original_query: originalQuery,
        fiscal_year: args.fiscal_year,
        fiscal_quarter: args.fiscal_quarter,
        months: args.months,
  ...
  if (toolName === "run_formula_metric") {
    ...
    const originalQuery = String(args.original_query ?? "");
    if (
      ["spm_forecast_attainment", "spm_pg_contribution", "spm_most_used_sales_play_programs"].includes(metricId) &&
    ...
    if (metricId === "spm_forecast_attainment") {
      ...
      binds = explicitMonthsProvided
        ...
        : [year2, year4, quarterDigit, year2, year4, quarterDigit];
    } else if (metricId === "spm_most_used_sales_play_programs") {
      const topN = typeof args.top_n === "number" ? Math.max(1, Math.min(100, Math.floor(args.top_n))) : 50;
      const scopedMonths = explicitMonthsProvided ? months : defaultMonthsForQuarter(fiscalQuarter);
      const monthDigits = scopedMonths.map((m) => Number(m));
      const monthBindPlaceholders = monthDigits.map(() => "?").join(", ");

      const pgTable = await resolveWorkingTable([
        "SSE_DM_GDSO_PRD.AIO.GSP_SPM_PIPEGEN",
        "SSE_DM_GDSO_PRD.AIO.GSP_SPM_PG"
      ]);
      const acvTable = await resolveWorkingTable(["SSE_DM_GDSO_PRD.AIO.GSP_SPM_ACV"]);
      const pgYearCol = await resolveWorkingColumn(pgTable, ["STG_2_FLG_DT_YEAR_FISCAL", "FISCAL_FLIP_YEAR"]);
      const pgQuarterCol = await resolveWorkingColumn(pgTable, ["STG_2_FLG_DT_QUARTER_FISCAL", "FISCAL_FLIP_QTR"]);
      const pgMonthCol = await resolveWorkingColumn(pgTable, ["STG_2_FLG_DT_MONTH", "FISCAL_FLIP_MON_NUM_LABEL"]);

      const acvYearCol = await resolveWorkingColumn(acvTable, [
        "CLSD_DT_YEAR_FISCAL",
        "FISCAL_FLIP_YEAR",
        "FISCAL_CMPGN_STRT_YEAR"
      ]);
      const acvQuarterCol = await resolveWorkingColumn(acvTable, [
        "CLSD_DT_QUARTER_FISCAL",
        "FISCAL_FLIP_QTR",
        "FISCAL_CMPGN_STRT_QTR"
      ]);
      const acvMonthCol = await resolveWorkingColumn(acvTable, [
        "CLSD_DT_MONTH",
        "FISCAL_FLIP_MON_NUM_LABEL",
        "FISCAL_CMPGN_STRT_MON_LABEL"
      ]);

      const pgHierarchyCol = await resolveWorkingColumn(pgTable, ["SPM_HIER_LVL_4", "SPM_HIER_LEVEL_4"]);
      const pgSalesPlayNameCol = await resolveWorkingColumn(pgTable, ["SALES_PLAY_NAME"]);
      const pgProgramIdCol = await resolveWorkingColumn(pgTable, ["PROGRAM_ID"]);
      const pgSalesPlayIdCol = await resolveWorkingColumn(pgTable, ["SALES_PLAY_ID"]);
      const pgCloudAmtCol = await resolveWorkingColumn(pgTable, ["CLOUD_AMT"]);

      const acvHierarchyCol = await resolveWorkingColumn(acvTable, ["SPM_HIER_LVL_4", "SPM_HIER_LEVEL_4"]);
      const acvSalesPlayNameCol = await resolveWorkingColumn(acvTable, ["SALES_PLAY_NAME"]);
      const acvProgramIdCol = await resolveWorkingColumn(acvTable, ["PROGRAM_ID"]);
      const acvSalesPlayIdCol = await resolveWorkingColumn(acvTable, ["SALES_PLAY_ID"]);
      const acvCloudAmtCol = await resolveWorkingColumn(acvTable, ["CLOUD_AMT"]);
      ...
      sqlText = `WITH pg_base AS (
        SELECT
          ${pgHierarchyCol} AS SPM_HIER_LVL_4,
          ${pgSalesPlayNameCol} AS SALES_PLAY_NAME,
          ${pgProgramIdCol} AS PROGRAM_ID,
          ${pgSalesPlayIdCol} AS SALES_PLAY_ID,
          ${pgCloudAmtCol} AS CLOUD_AMT
        FROM ${pgTable}
        WHERE PRODUCT_MATCH = 'Product Match'
          AND PROGRAM_TEAM = 'Sales Program'
          AND REGEXP_SUBSTR(TO_VARCHAR(${pgYearCol}), '[0-9]{2,4}') IN (?, ?)
          AND REGEXP_SUBSTR(TO_VARCHAR(${pgQuarterCol}), '[1-4]') = ?
          AND TRY_TO_NUMBER(REGEXP_SUBSTR(TO_VARCHAR(${pgMonthCol}), '[0-9]{1,2}')) IN (${monthBindPlaceholders})
      ...
      pg AS (
        SELECT
          ...
          SALES_PLAY_NAME,
          PROGRAM_ID,
          COUNT(DISTINCT PROGRAM_ID) AS TOT_PROGRAMS,
          COUNT(DISTINCT SALES_PLAY_ID) AS IND_SALES_PLAY,
          SUM(COUNT(DISTINCT SALES_PLAY_ID)) OVER (PARTITION BY PROGRAM_ID) AS TOT_SALES_PLAY,
          SUM(CLOUD_AMT) AS PIPEGEN,
          SUM(AVG(CLOUD_AMT)) OVER (PARTITION BY SPM_HIER_LVL_4) AS TOT_SPM_PG
        FROM pg_base
        GROUP BY SPM_HIER_LVL_4, SALES_PLAY_NAME, PROGRAM_ID
      ),
      res_base AS (
        SELECT SPM_HIER_LVL_4, SALES_PLAY_NAME, PROGRAM_ID, SALES_PLAY_ID FROM pg_base
      ),
      res AS (
        SELECT
          SPM_HIER_LVL_4,
      ...
      pg_sp_base AS (
        SELECT
          COALESCE(res.SPM_HIER_LVL_4, pg.SPM_HIER_LVL_4) AS SPM_HIER_LVL_4,
          COALESCE(res.PROGRAM_ID, pg.PROGRAM_ID) AS PROGRAM_ID,
          COALESCE(res.SALES_PLAY_NAME, pg.SALES_PLAY_NAME) AS SALES_PLAY_NAME,
          COALESCE(SUM(res.TOT_PROGRAMS), SUM(pg.TOT_PROGRAMS)) AS TOT_PROGRAMS,
          SUM(res.IND_SALES_PLAY) / NULLIF(SUM(res.TOT_SALES_PLAY), 0) AS WEIGHT_PROGRAM_COUNT,
          (SUM(pg.IND_SALES_PLAY) / NULLIF(SUM(pg.TOT_SALES_PLAY), 0)) * SUM(pg.PIPEGEN) AS WEIGHTED_PG
        FROM res
        FULL OUTER JOIN pg
          ON res.SPM_HIER_LVL_4 = pg.SPM_HIER_LVL_4
         AND res.SALES_PLAY_NAME = pg.SALES_PLAY_NAME
         AND res.PROGRAM_ID = pg.PROGRAM_ID
        GROUP BY
          COALESCE(res.SPM_HIER_LVL_4, pg.SPM_HIER_LVL_4),
          COALESCE(res.PROGRAM_ID, pg.PROGRAM_ID),
          COALESCE(res.SALES_PLAY_NAME, pg.SALES_PLAY_NAME)
      ),
      pg_sp AS (
        SELECT
          SPM_HIER_LVL_4,
          SALES_PLAY_NAME,
          SUM(TOT_PROGRAMS) AS TOT_PROGRAMS,
          SUM(WEIGHT_PROGRAM_COUNT) AS WEIGHT_PROGRAM_COUNT,
          SUM(WEIGHTED_PG) AS WEIGHTED_PG
        FROM pg_sp_base
        GROUP BY SPM_HIER_LVL_4, SALES_PLAY_NAME
      ),
      acv_base AS (
        SELECT
          ${acvHierarchyCol} AS SPM_HIER_LVL_4,
          ${acvSalesPlayNameCol} AS SALES_PLAY_NAME,
          ${acvProgramIdCol} AS PROGRAM_ID,
          ${acvSalesPlayIdCol} AS SALES_PLAY_ID,
          ${acvCloudAmtCol} AS CLOUD_AMT
        FROM ${acvTable}
        WHERE PRODUCT_MATCH = 'Product Match'
          AND PROGRAM_TEAM = 'Sales Program'
          AND REGEXP_SUBSTR(TO_VARCHAR(${acvYearCol}), '[0-9]{2,4}') IN (?, ?)
          AND REGEXP_SUBSTR(TO_VARCHAR(${acvQuarterCol}), '[1-4]') = ?
          AND TRY_TO_NUMBER(REGEXP_SUBSTR(TO_VARCHAR(${acvMonthCol}), '[0-9]{1,2}')) IN (${monthBindPlaceholders})
      ),
      acv AS (
        SELECT
          SPM_HIER_LVL_4,
          ...
          COUNT(DISTINCT SALES_PLAY_ID) AS IND_SALES_PLAY,
          SUM(COUNT(DISTINCT SALES_PLAY_ID)) OVER (PARTITION BY PROGRAM_ID) AS TOT_SALES_PLAY,
          SUM(CLOUD_AMT) AS ACV
        FROM acv_base
        GROUP BY SPM_HIER_LVL_4, SALES_PLAY_NAME, PROGRAM_ID
      ),
      acv_sp_base AS (
        SELECT
          COALESCE(res.SPM_HIER_LVL_4, acv.SPM_HIER_LVL_4) AS SPM_HIER_LVL_4,
          COALESCE(res.PROGRAM_ID, acv.PROGRAM_ID) AS PROGRAM_ID,
          COALESCE(res.SALES_PLAY_NAME, acv.SALES_PLAY_NAME) AS SALES_PLAY_NAME,
          COALESCE(SUM(res.TOT_PROGRAMS), SUM(acv.TOT_PROGRAMS)) AS TOT_PROGRAMS,
          SUM(res.IND_SALES_PLAY) / NULLIF(SUM(res.TOT_SALES_PLAY), 0) AS WEIGHT_PROGRAM_COUNT,
          (SUM(acv.IND_SALES_PLAY) / NULLIF(SUM(acv.TOT_SALES_PLAY), 0)) * SUM(acv.ACV) AS WEIGHTED_ACV
        FROM res
        FULL OUTER JOIN acv
          ON res.SPM_HIER_LVL_4 = acv.SPM_HIER_LVL_4
         AND res.SALES_PLAY_NAME = acv.SALES_PLAY_NAME
         AND res.PROGRAM_ID = acv.PROGRAM_ID
        GROUP BY
          COALESCE(res.SPM_HIER_LVL_4, acv.SPM_HIER_LVL_4),
          COALESCE(res.PROGRAM_ID, acv.PROGRAM_ID),
          COALESCE(res.SALES_PLAY_NAME, acv.SALES_PLAY_NAME)
      ),
      acv_sp AS (
        SELECT
          SPM_HIER_LVL_4,
          ...
          SUM(TOT_PROGRAMS) AS TOT_PROGRAMS,
          SUM(WEIGHT_PROGRAM_COUNT) AS WEIGHT_PROGRAM_COUNT,
          SUM(WEIGHTED_ACV) AS WEIGHTED_ACV
        FROM acv_sp_base
        GROUP BY SPM_HIER_LVL_4, SALES_PLAY_NAME
      )
      SELECT
        COALESCE(pg_sp.SPM_HIER_LVL_4, acv_sp.SPM_HIER_LVL_4) AS SPM_HIER_LVL_4,
        COALESCE(pg_sp.SALES_PLAY_NAME, acv_sp.SALES_PLAY_NAME) AS SALES_PLAY_NAME,
        COALESCE(SUM(pg_sp.TOT_PROGRAMS), SUM(acv_sp.TOT_PROGRAMS)) AS PROGRAM_COUNT,
        COALESCE(SUM(pg_sp.WEIGHT_PROGRAM_COUNT), SUM(acv_sp.WEIGHT_PROGRAM_COUNT)) AS WEIGHT_PROGRAM_COUNT,
        SUM(acv_sp.WEIGHTED_ACV) AS WEIGHTED_ACV,
        SUM(pg_sp.WEIGHTED_PG) AS WEIGHTED_PG
      FROM pg_sp
      FULL OUTER JOIN acv_sp
        ON pg_sp.SPM_HIER_LVL_4 = acv_sp.SPM_HIER_LVL_4
       AND pg_sp.SALES_PLAY_NAME = acv_sp.SALES_PLAY_NAME
      GROUP BY
        COALESCE(pg_sp.SPM_HIER_LVL_4, acv_sp.SPM_HIER_LVL_4),
        COALESCE(pg_sp.SALES_PLAY_NAME, acv_sp.SALES_PLAY_NAME)
      ORDER BY SPM_HIER_LVL_4 ASC, WEIGHTED_ACV DESC NULLS LAST
      LIMIT ${topN}`;
      binds = [
        year2,
        year4,
...
function buildSemanticRoute(
  ...
  if (
    q.includes("most used sales play") ||
    q.includes("sales play tagged in number of programs") ||
    (q.includes("sales play") && q.includes("number of programs"))
  ) {
    if (!fiscalYear || !fiscalQuarter) {
      return {
        intent_id: "most_used_sales_play_programs",
        route_tool: "run_most_used_sales_play_programs",
        arguments: {},
        clarification_needed: true,
        message:
          "Please provide fiscal_year and fiscal_quarter for this sales play program-usage metric (example: FY27 Q2)."
      };
    }
    return {
      intent_id: "most_used_sales_play_programs",
...
function isScopeConfirmedByQuery(query: string, fiscalYear: string, fiscalQuarter: string): boolean {
  ...
  const hasYear =
    !!fy2 &&
    (new RegExp(`\\bfy\\s*${fy2}\\b`).test(q) ||
      (!!fy4 && new RegExp(`\\bfy\\s*${fy4}\\b`).test(q)) ||
      (!!fy4 && new RegExp(`\\b${fy4}\\b`).test(q)));
  const hasQuarter = !!fq && new RegExp(`\\b(?:f\\s*)?q(?:uarter)?\\s*${fq}\\b`).test(q);
