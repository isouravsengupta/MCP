...
const spiDir = process.env.SPI_MCP_DIR ?? path.join(baseDir, "sales_planning_intelligence", "SPI MCP");
...
function metricAliases(name, id) {
  ...
  if (source.includes("most used sales play")) aliases.add("most used sales play");
  if (source.includes("number of programs")) aliases.add("sales play tagged in number of programs");
...
async function main() {
  ...
  const semanticCatalog = {
    ...
    intents: [
      ...
      {
        id: "most_used_sales_play_programs",
        description: "Most-used sales play tagged in number of programs with weighted ACV/PG.",
        match_phrases: [
          "most used sales play",
          "sales play tagged in number of programs",
          "sales play by number of programs"
        ],
