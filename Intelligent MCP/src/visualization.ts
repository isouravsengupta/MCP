...
function inferXKey(sampleRow: Record<string, unknown>): string | undefined {
...
function inferNumericKeys(sampleRow: Record<string, unknown>): string[] {
...
export function buildVisualizationPayload(
  ...
  const normalizedRows = rows.filter((r): r is Record<string, unknown> => typeof r === "object" && r !== null);
