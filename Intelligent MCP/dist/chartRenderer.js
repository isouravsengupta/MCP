...
export async function renderChartToPng(viz) {
    ...
    const datasets = (viz.yKeys ?? []).map((key, i) => ({
        ...
        values: rows.map((r) => (typeof r[key] === "number" ? r[key] : 0)),
    }));
    ...
    labels.forEach((label, i) => {
        const x = viz.component === "line_chart"
...
export async function renderAndUploadChart(viz, key) {
    ...
    try {
        ...
        return await getSignedUrl(getS3(), new GetObjectCommand({ Bucket: bucket, Key: s3Key }), { expiresIn: 3600 });
    }
    catch (err) {
