...
import { loadColumnDictionary, loadDashboardDatasetRegistry, loadDashboardLensCatalog, loadFormulaMetrics, loadSemanticCatalog, loadContextAssetIndex, loadContextRegistry, loadMetricCatalog, loadMetricRegressionSpec, loadMetricScenarioCatalog } from "./resources.js";
...
export async function handleMcpRequest(request, slackUserId) {
    try {
                ...
                return ok(request.id, {
                    tools: [
                        ...
                        },
                        {
                            ...
                            description: "Find top sales play by program count for an SPM hierarchy member (for example L4 person).",
                            inputSchema: {
                                type: "object",
                                properties: {
                        ...
                        },
                        {
                            ...
                            description: "Preferred path for 'forecast attainment' questions. Runs CRMA-equivalent formula metric (not SPM performance rollups).",
                            inputSchema: {
                                type: "object",
                                properties: {
                                    fiscal_year: { type: "string" },
                        ...
                        },
                        {
                            ...
                            description: "Preferred path for 'PG Contribution (%)' questions. Runs CRMA-equivalent PG contribution metric.",
                            inputSchema: {
                                type: "object",
                                properties: {
                                    fiscal_year: { type: "string" },
                        ...
                        },
                        {
                            ...
                            description: "Look up an opportunity by ID directly in SPI Snowflake datasets and return available amount fields.",
                            inputSchema: {
                                type: "object",
                                properties: {
...
async function handleToolCall(params, snowflake) {
    ...
    if (toolName === "snowflake_connectivity_probe") {
        const rows = await snowflake.execute("SELECT CURRENT_USER() AS USER_NAME, CURRENT_ROLE() AS ROLE_NAME, CURRENT_WAREHOUSE() AS WH_NAME, CURRENT_IP_ADDRESS() AS IP_ADDR");
        return {
    ...
    if (toolName === "dashboard_usecase_coverage") {
        ...
        const datasetResolution = new Map();
            ...
            content: [
                {
                    ...
                    text: JSON.stringify({
                        ...
                        dataset_resolution: Object.fromEntries([...datasetResolution.entries()].map(([k, v]) => [
                            k,
    ...
    if (toolName === "query_dashboard_dataset") {
        ...
        const sampleRows = await snowflake.execute(`SELECT * FROM ${table} LIMIT 1`);
        const available = new Set(Object.keys((sampleRows[0] ?? {})).map((c) => c.toUpperCase()));
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            clarification_needed: true,
                            message: "Please confirm fiscal scope in your question (example: 'Forecast attainment FY27 Q2')."
                        }, null, 2)
                    }
                ]
            };
    ...
    if (toolName === "lookup_opportunity_amount") {
        ...
        for (const table of tableCandidates) {
            ...
            const rows = await snowflake.execute(`SELECT * FROM ${resolvedTable}
         WHERE ${idCol} = ? OR LEFT(${idCol}, 15) = ?
         LIMIT 1`, [opportunityId, opp15]);
            if (rows.length === 0) {
                const candidates = await snowflake.execute(`SELECT DISTINCT ${idCol} AS OPP_ID
           FROM ${resolvedTable}
           WHERE ${idCol} ILIKE ?
           ORDER BY 1
           LIMIT 5`, [`${opp15.slice(0, 8)}%`]);
            ...
            const row = (rows[0] ?? {});
            const numericAmountFields = Object.fromEntries(Object.entries(row).filter(([k, v]) => /(?:^|_)(AMT|AMOUNT|ACV|CLOUD)(?:_|$)/i.test(k) && typeof v === "number"));
            return {
                content: [
                    {
                        type: "text",
    ...
    if (toolName === "run_formula_metric") {
        ...
        const months = explicitMonthsProvided ? args.months.map((m) => String(m)) : [];
        if (!metricId || !fiscalYear || !fiscalQuarter) {
            throw new Error("metric_id, fiscal_year, and fiscal_quarter are required.");
        }
        const originalQuery = String(args.original_query ?? "");
        if (["spm_forecast_attainment", "spm_pg_contribution"].includes(metricId) &&
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            clarification_needed: true,
                            message: "For this financial formula metric, fiscal scope must be explicitly present in the user query (for example FY27 Q2)."
                        }, null, 2)
                    }
                ]
            };
      ...
      SELECT
        ...
        COALESCE((SELECT SUM(FORECAST_AMT) FROM SPM_FRCST),0) AS FORECAST_AMT`;
            ...
            const pgMonthPredicate = explicitMonthsProvided && pgMonthCol
                ? `AND TRY_TO_NUMBER(REGEXP_SUBSTR(TO_VARCHAR(${pgMonthCol}), '[0-9]{1,2}')) IN (${monthBindPlaceholders})`
                : "";
            const totalMonthPredicate = explicitMonthsProvided && totalMonthCol
    ...
    if (toolName === "resolve_sales_play_for_hierarchy") {
        ...
        const hierarchyCandidates = hierarchyLevel === "SPM_HIER_LVL_4"
        ...
        const resolvedHierarchyColumn = await resolveWorkingColumn("SSE_DM_GDSO_PRD.AIO.GSP_SPM_TARGET_ACCTS", hierarchyCandidates, snowflake);
        const resolvedFiscalYearColumn = await resolveWorkingColumnOptional("SSE_DM_GDSO_PRD.AIO.GSP_SPM_TARGET_ACCTS", fiscalYearCandidates, snowflake);
        const resolvedSalesPlayColumn = await resolveWorkingColumn("SSE_DM_GDSO_PRD.AIO.GSP_SPM_TARGET_ACCTS", salesPlayCandidates, snowflake);
        const nameMatches = await snowflake.execute(`SELECT DISTINCT ${resolvedHierarchyColumn} AS PERSON_NAME
       FROM SSE_DM_GDSO_PRD.AIO.GSP_SPM_TARGET_ACCTS
       WHERE ${resolvedHierarchyColumn} ILIKE ?
       ORDER BY 1
       LIMIT 10`, [`%${personName}%`]);
        const exact = nameMatches.find((row) => String(row.PERSON_NAME ?? "") === personName);
        if (!exact) {
            return {
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            suggestions: nameMatches.map((row) => row.PERSON_NAME)
                        }, null, 2)
                    }
                ]
            };
        ...
        const rows = await snowflake.execute(`SELECT ${resolvedSalesPlayColumn} AS SALES_PLAY_NAME, COUNT(DISTINCT PROGRAM_ID) AS PROGRAM_COUNT
       FROM SSE_DM_GDSO_PRD.AIO.GSP_SPM_TARGET_ACCTS
    ...
    if (!metric) {
        if (metricId.toLowerCase().includes("forecast_attainment")) {
            throw new Error("Forecast attainment is a formula metric. Use run_forecast_attainment or run_formula_metric(metric_id='spm_forecast_attainment', fiscal_year, fiscal_quarter).");
        }
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            clarification_needed: true,
                            message: "Query returned low-signal output (empty/all-null). Please confirm scope to avoid misleading results.",
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            clarification_needed: true,
                            message: "One or more filter values did not match table data. Please pick one of the suggested values.",
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            clarification_needed: true,
                            message: "Result set is empty or low-signal. Please refine filters before final output.",
                            clarifying_questions: [
...
async function resolveDatasetTable(dataset, registry, snowflake) {
    const entry = registry.datasets.find((d) => d.dataset === dataset);
    const candidates = new Set(entry?.table_candidates ?? []);
...
function normalizeFiscalQuarter(input) {
    const s = input.trim().toUpperCase();
    if (!s)
        return "";
    if (s.startsWith("FQ "))
...
function defaultMonthsForQuarter(fiscalQuarter) {
...
function buildSemanticRoute(input, semanticCatalog) {
    ...
    const financialIntent = q.includes("contribution") ||
    ...
    if (financialIntent && (!fiscalYear || !fiscalQuarter)) {
        return {
            ...
            arguments: {},
            clarification_needed: true,
            message: "Please confirm fiscal scope before I run this financial metric. Share fiscal_year (for example FY27) and fiscal_quarter (for example Q1/Q2)."
        };
    }
    const scored = semanticCatalog.intents
        .map((intent) => {
        const score = intent.match_phrases.reduce((acc, phrase) => (q.includes(phrase.toLowerCase()) ? acc + Math.max(phrase.length, 1) : acc), 0);
        return { intent, score };
    })
        .filter((row) => row.score > 0)
        .sort((a, b) => b.score - a.score);
    ...
    if (top && second && second.score === scored[0].score && scored[0].score > 0) {
        return {
            ...
            arguments: {},
            clarification_needed: true,
            message: "I found multiple matching metrics for this query. Please specify the exact metric name shown in dashboard."
        };
    }
    if (top) {
    ...
    return {
        ...
        arguments: {},
        clarification_needed: true,
        message: "Could not deterministically route this query yet. Please provide metric name or dataset/measure to avoid wrong answers."
    };
}
function extractFiscalScope(query) {
...
function isScopeConfirmedByQuery(query, fiscalYear, fiscalQuarter) {
    const q = query.toLowerCase();
    if (!q.trim())
        return false;
...
function withFiscalYearFixedFiltersRemoved(scenarioCatalog, metricId) {
    return {
        scenarios: scenarioCatalog.scenarios.map((scenario) => {
            if (scenario.metric_id !== metricId || !scenario.fixed_filters) {
                return scenario;
            }
            const filtered = Object.fromEntries(Object.entries(scenario.fixed_filters).filter(([k]) => !k.toUpperCase().includes("FISCAL_YEAR")));
            return { ...scenario, fixed_filters: filtered };
        })
    };
}
function assessResultQuality(rows) {
    if (rows.length === 0) {
    ...
    const numericKeys = Object.keys(objectRows[0]).filter((k) => objectRows.some((row) => typeof row[k] === "number" || row[k] === null));
    if (numericKeys.length === 0) {
        return { clarificationNeeded: false, reason: null, rowCount: rows.length };
    }
    const allNullOrZero = numericKeys.every((key) => objectRows.every((row) => row[key] === null || row[key] === 0));
    if (allNullOrZero) {
        return { clarificationNeeded: true, reason: "all numeric measures are null/zero", rowCount: rows.length };
    }
    return { clarificationNeeded: false, reason: null, rowCount: rows.length };
}
function defaultClarifyingQuestions(availableFilterMap) {
...
async function resolveWorkingColumn(tableName, candidates, snowflake) {
...
async function resolveWorkingColumnOptional(tableName, candidates, snowflake) {
    for (const candidate of candidates) {
        try {
            await snowflake.execute(`SELECT ${candidate} FROM ${tableName} LIMIT 1`);
            return candidate;
...
async function resolveWorkingTable(candidates, snowflake) {
    for (const candidate of candidates) {
        try {
...
async function validateFilters(input) {
    ...
    for (const [filterKey, filterValue] of Object.entries(input.filters)) {
        ...
        const exactRows = await input.snowflake.execute(`SELECT COUNT(1) AS CNT FROM ${input.sourceTable} WHERE ${column} = ?`, [filterValue]);
        const exactCount = Number(exactRows[0]?.CNT ?? 0);
        if (exactCount > 0) {
            continue;
        }
        const suggestionRows = await input.snowflake.execute(`SELECT DISTINCT ${column} AS VALUE FROM ${input.sourceTable} WHERE ${column} ILIKE ? ORDER BY 1 LIMIT 8`, [`%${filterValue}%`]);
