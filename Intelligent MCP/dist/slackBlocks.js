...
function barChartBlocks(viz) {
    const rows = viz.rowsPreview.slice(0, 10);
    if (!viz.xKey || !viz.yKeys?.length || rows.length === 0)
        return tableBlocks(viz);
    const yKey = viz.yKeys[0];
    const maxVal = Math.max(...rows.map((r) => (typeof r[yKey] === "number" ? r[yKey] : 0)));
    const BAR_WIDTH = 20;
    const lines = rows.map((r) => {
        const label = String(r[viz.xKey] ?? "").slice(0, 16).padEnd(16);
        const val = typeof r[yKey] === "number" ? r[yKey] : 0;
...
function yoyBlocks(viz) {
    ...
    const lines = rows.map((r) => {
        ...
        const yoy = yoyKey && typeof r[yoyKey] === "number"
            ? ` ${r[yoyKey] >= 0 ? "▲" : "▼"} ${Math.abs(r[yoyKey] * 100).toFixed(1)}% YoY`
            : "";
...
export function buildSlackBlocks(viz, chartImageUrl) {
    let blocks;
